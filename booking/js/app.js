...
function applyLang() {
  ...
  // Re-render tours for translated book btn
...
const LOCAL_LOCS = [
  {label:'Amsterdam Centraal Station',icon:'train',zone:'centrum'},{label:'Schiphol Airport, Luchthaven',icon:'plane',zone:'schiphol'},{label:'Amsterdam Noord (NDSM-Werf)',icon:'pin',zone:'noord'},{label:'Amsterdam Oost (Watergraafsmeer)',icon:'pin',zone:'oost'},{label:'Amsterdam Zuid (Rivierenbuurt)',icon:'pin',zone:'zuid'},{label:'Amsterdam West (Jordaan)',icon:'pin',zone:'west'},{label:'Amsterdam Nieuw-West',icon:'pin',zone:'nieuwwest'},{label:'Amsterdam Zuidoost (Bijlmer)',icon:'pin',zone:'zuidoost'},{label:'Amsterdam RAI',icon:'building',zone:'rai'},{label:'Ziggo Dome, Amsterdam',icon:'building',zone:'zuidoost'},{label:'Johan Cruyff ArenA, Amsterdam',icon:'building',zone:'zuidoost'},{label:'Amsterdam UMC (AMC)',icon:'hospital',zone:'zuidoost'},{label:'OLVG Ziekenhuis Oost, Amsterdam',icon:'hospital',zone:'oost'},{label:'Haarlem Centrum',icon:'city',zone:'haarlem'},{label:'Utrecht Centraal',icon:'train',zone:'utrecht'},{label:'Den Haag Centraal',icon:'train',zone:'denhaag'},{label:'Rotterdam Centraal',icon:'train',zone:'rotterdam'},{label:'Almere Centrum',icon:'city',zone:'almere'},{label:'Zaandam (Zaanse Schans)',icon:'pin',zone:'zaandam'},{label:'Amstelveen Centrum',icon:'city',zone:'amstelveen'},{label:'Keukenhof, Lisse',icon:'pin',zone:'lisse'},{label:'Volendam Haven',icon:'pin',zone:'volendam'},{label:'Diemen Centrum',icon:'pin',zone:'diemen'},
];
...
function initTabs() {
  document.querySelectorAll('[data-tab-btn]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (target === 'boek') setTimeout(() => map?.invalidateSize(), 50);
      window.scrollTo(0, 0);
...
function initMap() {
  ...
  document.getElementById('myloc-btn')?.addEventListener('click', ()=>{
    ...
    }, ()=>{ btn.style.color=''; },{enableHighAccuracy:true,timeout:8000});
  });
...
function drawRouteLine() {
  ...
  routeLine = L.polyline([pickupMarker.getLatLng(),dropoffMarker.getLatLng()],{color:'#C8A84C',weight:2,opacity:0.6,dashArray:'6,5'}).addTo(map);
}
...
function setupAutocomplete(inputId,suggestId) {
  ...
  const filterLocal = q => { const t=q.toLowerCase(); return LOCAL_LOCS.filter(l=>l.label.toLowerCase().includes(t)).slice(0,5); };
  ...
  function renderSuggestions(local,api) {
    ...
    box.innerHTML = all.slice(0,7).map((l,i)=>`<div class="suggest-item" data-idx="${i}" data-label="${escHtml(l.label)}" ${l.lat?`data-lat="${l.lat}" data-lon="${l.lon}"`:''}><div class="suggest-icon">${iconSvg(l.icon)}</div><span>${l.label}</span></div>`).join('');
...
function iconSvg(type) {
  const m={pin:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5a2.5 2.5 0 110-5 2.5 2.5 0 010 5z"/></svg>',train:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2c-4 0-8 .5-8 4v9.5C4 17.43 5.57 19 7.5 19L6 20.5v.5h12v-.5L16.5 19c1.93 0 3.5-1.57 3.5-3.5V6c0-3.5-4-4-8-4zm0 2c3.51 0 4.96.48 5.57 1H6.43C7.04 4.48 8.49 4 12 4zm6 10H6v-4h12v4zm-8.5 3a1.5 1.5 0 110-3 1.5 1.5 0 010 3zm5 0a1.5 1.5 0 110-3 1.5 1.5 0 010 3z"/></svg>',plane:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M21 16v-2l-8-5V3.5a1.5 1.5 0 00-3 0V9l-8 5v2l8-2.5V19l-2 1.5V22l3.5-1 3.5 1v-1.5L13 19v-5.5z"/></svg>',building:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 7V3H2v18h20V7H12zM6 19H4v-2h2v2zm0-4H4v-2h2v2zm0-4H4V9h2v2zm0-4H4V5h2v2zm4 12H8v-2h2v2zm0-4H8v-2h2v2zm0-4H8V9h2v2zm0-4H8V5h2v2zm10 12h-8v-2h2v-2h-2v-2h2v-2h-2V9h8v10zm-2-8h-2v2h2v-2zm0 4h-2v2h2v-2z"/></svg>',hospital:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M19 3H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2V5a2 2 0 00-2-2zm-2 10h-4v4h-2v-4H7v-2h4V7h2v4h4v2z"/></svg>',city:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M15 11V5l-3-3-3 3v2H3v14h18V11h-6zm-8 8H5v-2h2v2zm0-4H5v-2h2v2zm0-4H5V9h2v2zm6 8h-2v-2h2v2zm0-4h-2v-2h2v2zm0-4h-2V9h2v2zm0-4h-2V5h2v2zm6 12h-2v-2h2v2zm0-4h-2v-2h2v2z"/></svg>'};
  return m[type]||m.pin;
}
...
function initForm() {
  document.getElementById('svc-tabs')?.querySelectorAll('.svc-tab').forEach(btn=>{
    btn.addEventListener('click',()=>{
      ...
      if (btn.dataset.svc==='airport') { const d=document.getElementById('dropoff'); if (d&&!d.value) { d.value='Schiphol Airport, Luchthaven'; updatePrice(); } }
    });
  });
  ...
  document.querySelectorAll('.pin-btn').forEach(btn=>{
    btn.addEventListener('click',()=>{
      ...
      if (panel&&window.innerWidth<768&&!panel.classList.contains('map-open')) {
        ...
        document.getElementById('map-toggle-label').textContent=T('map_hide');
  ...
  document.getElementById('book-btn')?.addEventListener('click',()=>{
    const msg=buildMsg({pickup:document.getElementById('pickup')?.value.trim(),dropoff:document.getElementById('dropoff')?.value.trim(),date:document.getElementById('date')?.value,time:document.getElementById('time')?.value,retDate:document.getElementById('return-date')?.value,retTime:document.getElementById('return-time')?.value,pax:document.getElementById('passengers')?.value,vehicle:document.getElementById('vehicle')?.value,service:document.querySelector('.svc-tab.active')?.dataset.svc});
    window.open(`https://wa.me/${PHONE}?text=${msg}`,'_blank','noopener');
  });
...
function initCountUp() {
  const obs=new IntersectionObserver(entries=>{
    entries.forEach(e=>{
      ...
      (function tick(now){ const p=Math.min((now-start)/1200,1); el.textContent=Math.round((1-Math.pow(1-p,3))*target); if(p<1) requestAnimationFrame(tick); else el.textContent=target; })(start);
      obs.unobserve(el);
    });
  },{threshold:0.5});
