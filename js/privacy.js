...
export function initPrivacyToggle(container) {
  ...
  if (!btn) return;
  const update = () => {
    container.querySelectorAll('.blurred-amount').forEach(el => el.classList.toggle('revealed', _revealed));
    btn.innerHTML = _revealed ? EYE_ON : EYE_OFF;
    btn.title = _revealed ? 'Verberg bedragen' : 'Toon bedragen';
    btn.setAttribute('aria-label', _revealed ? 'Verberg bedragen' : 'Toon bedragen');
  };
  btn.onclick = e => { e.stopPropagation(); _revealed = !_revealed; update(); };
  update();
}
